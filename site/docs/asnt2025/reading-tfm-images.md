...
To learn how to display a TFM image from an .nde data file, follow the steps outlined below, based on this  [example file](../examples/example-files/index.md#manual-weld-scanning-using-the-total-focusing-method-tfm) of a manual weld scan using the total focusing method (TFM). 
...
Group ID: 1, Group Name: TT
TFM Path Name: T-T
  Dataset ID: 0, Data Class: TfmValue, Data Path: /Public/Groups/1/Datasets/0-TfmValue
     Axis: UCoordinate, Offset: 0.031, Quantity: 12, Resolution: 0.001
...
As an example, for the T-T waveset, we now know that we have 12 increments against the U axis (spaced by 1 mm) and that each TFM image has a dimension of 119 (V axis) x 80 (W axis) pixels. The vertical resolution of each image is 1.58 mm and the horizontal resolution is 1.69 mm. 

Let's plot the 10th TFM image of this scan: 
...
# Show plot
plt.title(f'T-T TFM Image #{TFM_U_increment+1}')
plt.show()
```

You should end up with the following figure:

![tfm_image](images/tfm_image.png)
